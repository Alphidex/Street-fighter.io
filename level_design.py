...
class Level_Design:
    def __init__(self, player_1, player_2, fighters_pos, map_instance):
        # Display
    ...
    def display_fight_text(self):
        font = pygame.font.Font("./Fonts/retro.ttf", 80)
        round_text = font.render(f"Round: {self.current_round}", False, "white")
        round_text_pos = (500, 300)
        fight_text = font.render(f"Fight!", False, "white")
        fight_text_pos = (500, 300)
    ...
    def check_for_won_rounds(self, p1_health, p2_health):
        ...
        if self.round_over:
            ...
            text = font.render("You Won!", False, "green")

            if not self.do_once_round:
                self.round_won_timer = pygame.time.get_ticks()
                self.do_once_round = True
            
            if pygame.time.get_ticks() - self.round_won_timer < 1200 and self.p1_rounds_won <= 1:
                self.screen.blit(text, (500, 300))
                
            else:
                if self.p1_rounds_won <= 1:
                    self.displayed_labels = 0
                    self.fight_countdown_complete = False
                self.do_once_round = False
    ...
    def game_won(self):
        if self.current_match >= self.max_matches and self.p1_rounds_won >= 2:
            ...
            text = font.render("Congratulations!", False, "green")
