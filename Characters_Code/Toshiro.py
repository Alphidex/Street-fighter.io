...
class Toshiro(Fighter):
    ...
    def attack(self, surface, target):
        ...
        if self.normal_attack:
            ...
            self.attacking_rectangle = pygame.Rect(self.rect.centerx - 80 - (10 * self.flip),
                                                   self.rect.y - 10, 170, self.rect.height - 40)

        # Strong Attack Attacks
        elif self.strong_attack:
            self.attacking_rectangle = pygame.Rect(self.rect.centerx - 70 - (120 * self.flip),
                                              self.rect.y - 25, 260, 1.2 * self.rect.height)
        elif self.strong_attack_up:
            self.attacking_rectangle = pygame.Rect(self.rect.centerx - 40 - (70 * self.flip),
                                              self.rect.y - 40, 150, 1 * self.rect.height)
        elif self.strong_attack_down:
            self.attacking_rectangle = pygame.Rect(self.rect.centerx - 10 - (130 * self.flip),
                                              self.rect.y + 20, 150, 140)
        elif self.strong_jump_attack:
            self.attacking_rectangle = pygame.Rect(self.rect.x - 30 - (140 * self.flip),
                                                   self.rect.y, 200, 1.4 * self.rect.height)
        # Special Attack Attacks
        elif self.special_attack:
            self.attacking_rectangle = pygame.Rect(self.rect.centerx - 100,
                                              self.rect.y - 50, 265, 1.3 * self.rect.height)
        elif self.special_attack_up:
            self.attacking_rectangle = pygame.Rect(self.rect.centerx - 20,
                                                   self.rect.y - 20, 160, 40 + self.rect.height)
        elif self.special_attack_down:
            self.attacking_rectangle = pygame.Rect(self.rect.centerx - 120,
        ...
        if self.attacking:
            ...
            if self.frame_index < len(self.animation_list[self.action]):
                ...
                if self.normal_attack:
                    ...
                    if self.attack_list_trigger[self.frame_index] == False and 4 <= self.frame_index <= 6:
                        self.attack(surface, target)
                        self.attack_list_trigger[self.frame_index] = True
                ...
                # Special Attack Attacks
                    ...
                    if self.attack_list_trigger[self.frame_index] == False and 16 <= self.frame_index <= 19:
                        if self.frame_index == 16:
                            init_ranged_attack(26)
                        self.attack(surface, target)
                        self.attack_list_trigger[self.frame_index] = True
    ...
    class Ranged_Toshiro(Ranged_Attack):
        ...
        def create_rectangle(self, character_rect):
            ...
            # Jump Strong Attack
            elif self.range_action == 19:
                rect = pygame.Rect(character_rect.centerx - (260 * self.range_flip),
                                   character_rect.y + 50, 260, 1.1 * character_rect.height)
            # Special Attack Up
            elif self.range_action == 18:
                rect = pygame.Rect(character_rect.centerx - (260 * self.range_flip),
                                   character_rect.y - 90, 200, 90 + character_rect.height)
            # Special Attack Down
            elif self.range_action == 26:
        ...
        def update_ranged_attack(self, surface):
            ...
            if self.range_action == 20:  # Strong Attack Down
                if self.range_frame_index >= 1 and not self.collision:
                    self.range_frame_index = 1
            if self.range_action == 18:  # Special Up
                if self.range_frame_index >= 2 and not self.collision:
                    self.range_frame_index = 2
            ...
            # Image movement
            if self.range_action == 24:  # Strong Attack Up
                self.rect.x += 2 + (-4 * self.range_flip)
                self.rect.y -= 1.8
            elif self.range_action == 20 and self.range_frame_index == 3:  # Strong Attack Down
                self.rect.x = 1650 - (3200 * self.range_flip)
            elif self.range_action == 19:  # Strong Jump
                self.rect.y += 2
                self.rect.x += 2 + (-4 * self.range_flip)
            elif self.range_action == 18 and self.range_frame_index == 4:  # Special Up
                self.rect.x = 1650 - (3200 * self.range_flip)
            elif self.range_action == 26:
                if self.range_frame_index == 2:  # Special Up
                    self.rect.x = 1650 - (3200 * self.range_flip)
            else:
                self.rect.x += 2 + (-4 * self.range_flip)
