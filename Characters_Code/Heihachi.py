...
class Heihachi(Fighter):
        ...
        # Normal Attack Down
            ...
            if not self.flip:
                surface.blit(img, (
                    ...
                    self.rect.y - ((self.offset[1] + 52) * self.image_scale)))
            else:
                surface.blit(img, (
    ...
    def get_attack_data(self):
        data = {
            "normal_attack": {"trigger": False, "cooldown": 80, "frame_index": 1, "damage": 1.5, "knockback": 1,
            ...
            "normal_attack_up": {"trigger": False, "cooldown": 75, "frame_index": 2, "damage": 1.3, "knockback": 1,
                                 "action": 13, "range_attack": False,
                                 "rect": pygame.Rect(self.rect.centerx + 25 - (120 * self.flip), self.rect.y - 100, 70,
                                                     1.35 * self.rect.height)},
            "normal_attack_down": {"trigger": False, "cooldown": 55, "frame_index": 1, "damage": 2, "knockback": 1,
                                   "action": 6, "range_attack": False,
                                   "rect": pygame.Rect(self.rect.centerx - 130, self.rect.y + 80, 260, 60)},
            "normal_jump_attack": {"trigger": False, "cooldown": 85, "frame_index": 3, "damage": 1.7, "knockback": 1,
                                   "action": 8, "range_attack": False,
                                   "rect": pygame.Rect(self.rect.centerx - 20 - (135 * self.flip), self.rect.y - 20,
                                                       175, self.rect.height + 40)},
            "strong_attack": {"trigger": False, "cooldown": 90, "frame_index": 2 <= self.frame_index <= 11, "damage": 0.7,
                              "knockback": 1, "action": 10, "range_attack": False,
                              "rect": pygame.Rect(self.rect.centerx - (150 * self.flip), self.rect.y + 10, 150,
                                                  0.75 * self.rect.height)},
            "strong_attack_up": {"trigger": False, "cooldown": 100, "frame_index": 6, "damage": 7, "knockback": 1,
            ...
            "strong_attack_down": {"trigger": False, "cooldown": 90, "frame_index": 4, "damage": 8, "knockback": 1,
                                   ...
                                   "rect": pygame.Rect(self.rect.centerx - 30 - (30 * self.flip), self.rect.y - 20, 90,
                                                       self.rect.height + 40)},
            "strong_jump_attack": {"trigger": False, "cooldown": 70, "frame_index": 3 <= self.frame_index <= 5,
                                   "damage": 1, "knockback": 1, "action": 7, "range_attack": False,
                                   "rect": pygame.Rect(self.rect.centerx - 30 - (60 * self.flip), self.rect.y, 120,
                                                       self.rect.height)},
            "special_attack": {"trigger": False, "cooldown": 110, "frame_index": 9, "damage": 22, "knockback": 1,
            ...
            "special_attack_up": {"trigger": False, "cooldown": 110, "frame_index": 0, "damage": 25, "knockback": 1,
                                  "action": 1, "range_attack": False,
                                  "rect": pygame.Rect(self.rect.centerx - 130, self.rect.y - 150, 260,
                                                      150 + self.rect.height)},
            "special_attack_down": {"trigger": False, "cooldown": 160, "frame_index": 10, "damage": 40, "knockback": 1,
                                    "action": 0, "range_attack": True,
                                    "rect": pygame.Rect(self.rect.centerx - 150 - (1 * self.flip), self.rect.y - 50,
                                                        300, 150 + self.rect.height)}
        }
        return data
    ...
    def attacks_management(self, surface, target):
        ...
        def attack_effects():
            ...
            if self.attack_data["special_attack_down"]["trigger"]:
                if 6 < self.frame_index < 9:
                    self.rect.x += 5 - (10 * self.flip)

                if self.attack_effect_executions == 0:
                    if self.frame_index == 6:
                        self.rect.y -= 400
                        self.rect.x = target.rect.x - 300 + (700 * self.flip)
                        ...
                        if self.frame_index_counter[1] - self.frame_index_counter[0] >= 1\
                            ...
                            if attack["range_attack"]:
                                ...
                                self.ranged_group.add(
                                    self.Ranged_Heihachi(self.flip, self.projectile_list, self.action, self.rect, "Heihachi",
                                                      self.opponent))
    ...
    class Ranged_Heihachi(Ranged_Attack):
        def __init__(self, flip, attack_animation_list, action, character_rect, character, opponent):
            ...
            self.attack_data = {
                3: {"name": "special_attack", "damage": 25},
                0: {"name": "strong_attack_down", "damage": 40},
                2: {"name": "strong_attack_up", "damage": 12},
                4: {"name": "strong_attack_down", "damage": 13}
            }
        ...
        def create_rectangle(self, character_rect):
            # Special Attack
            if self.action == 3:
                rect = pygame.Rect(character_rect.centerx + 10 - (2 * character_rect.width * self.flip),
                                   character_rect.y + 5, 1.8 * character_rect.width,
                                   0.9 * character_rect.height)

            # Special Attack Down
            if self.action == 0:
                rect = pygame.Rect(character_rect.centerx + 60 - (350 * self.flip),
        ...
        def update_ranged_attack(self, surface):
            ...
            if self.action == 2:  # Strong Attack Up
            ...
            elif self.action == 3:  # Special Attack
                # Increments the frame index
            ...
            elif self.action == 0:  # Special Attack Down
                # Increments the frame index
                if (pygame.time.get_ticks() - self.update_time) > 180:
                    self.frame_index += 1
                    self.update_time = pygame.time.get_ticks()
